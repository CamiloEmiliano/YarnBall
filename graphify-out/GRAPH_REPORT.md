...
## Corpus Check
- 38 files · ~12,917 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 369 nodes · 591 edges · 19 communities (17 shown, 2 thin omitted)
...
2. `_connect_db()` - 14 edges
3. `pg_connection()` - 12 edges
...
- `pg_connection()` --calls--> `_connect_db()`  [EXTRACTED]
  graph/db.py → tools/init_db.py
...
Cohesion: 0.12
Nodes (19): BaseModel, _DummyModel, embed_texts(), Return a list of embedding vectors for the supplied texts. The vectors are…, _build_dsn(), pg_connection(), Construct a clean DSN for PostgreSQL, respecting custom database names., Return a PostgreSQL connection. Prefers tools.init_db._connect_db, then falls… (+11 more)
...
- **Why does `_connect_db()` connect `scraping_worker.py` to `pg_connection`?**
